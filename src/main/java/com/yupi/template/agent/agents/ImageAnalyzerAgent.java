...
public class ImageAnalyzerAgent implements NodeAction {
    ...
    private static final String PROFILE_NAME = "image-analyzer";
    ...
    public Map<String, Object> apply(OverAllState state) {
        ...
        AgentProfile profile = agentPromptSupport.resolveProfile(PROFILE_NAME, "agent4_image", false, true);
        ...
        return switch (method) {
            ...
            case "SVG_DIAGRAM" -> """
                    - SVG_DIAGRAM: 在 prompt 字段描述示意图需求（中文），说明要表达的概念和关系。keywords 留空。
                      示例：绘制思维导图样式的图，中心是"自律"，周围 4 个分支：习惯、环境、反馈、系统
                    """;
            default -> null;
        };
    }
    ...
    private List<ArticleState.ImageRequirement> validateAndFilterImageRequirements(
        ...
        for (ArticleState.ImageRequirement req : requirements) {
            ...
            if (enabledMethods.contains(imageSource)) {
                ...
                validatedRequirements.add(req);
                log.info("Image requirement source replaced, position={}, fallback={}", req.getPosition(), fallbackSource);
            }
        }
