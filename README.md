...
| 🔍 执行观测与重放 | 节点日志、时间线、重试、重放调试 | 可排障、可解释、可恢复 |
| 🧠 长期记忆增强 | 历史成功 / 失败经验参与后续决策 | 越用越懂创作偏好 |
...
项目当前已经从“基础图文生成”升级到“Agent 工程化增强”阶段，新增能力包括：

- ✅ 节点级执行观测：记录 `node / status / elapsedMs / decisionSummary`
- ✅ 节点时间线与调试面板：前端可查看关键节点执行过程
- ✅ 失败节点重试：支持针对失败节点进行局部重跑
- ✅ 任务恢复：任务中断后可从快照状态继续推进
- ✅ 节点重放：支持按节点查看输入、输出、错误和决策信息
- ✅ 长期记忆第一版：历史成功 / 失败案例召回参与正文评审与图片决策
- ✅ 图片链路可解释化：图片策略路由、图片需求分析、图片生成与降级链路均可追踪
...
│   │   ├── ArticleGenerationWorkflowService.java # 统一工作流入口
│   │   ├── ArticleMemoryService.java # 长期记忆服务
│   │   ├── ArticleNodeReplayService.java # 节点重放服务
│   │   ├── ArticleTaskSnapshotService.java # 任务快照服务
│   │   ├── ArticleWorkflowEventService.java # 工作流事件服务
│   │   ├── ImageServiceStrategy.java# 配图策略选择器
│   │   ├── ImageStrategyRouterService.java # 图片策略路由
│   │   ├── ImageFallbackRouterService.java # 图片降级路由
...
│   ├── src/
│   │   ├── pages/                   # 页面组件
│   │   │   └── article/components/  # 节点重放、任务记忆等调试面板
│   │   ├── components/              # 公共组件
│   │   ├── api/                     # API 接口
│   │   └── stores/                  # 状态管理
│   └── package.json
...
当前后端已经补齐一套更完整的 Agent 工程化链路：

- 节点日志采集：记录节点状态、耗时、模型、Prompt、决策来源
- 任务快照：保存阶段状态，支持任务恢复
- 失败节点重试：避免整条链路从头执行
- 节点重放：查看某次节点执行时的输入、输出、错误与元数据
- 图片决策可解释：支持查看路由来源、降级原因和执行摘要
- 长期记忆上下文：支持查看记忆摘要与快照，理解 Agent 为什么这样决策

### 长期记忆第一版

当前已实现结构化长期记忆，不依赖向量库，主要用于提升 Agent 决策质量：

- 记录任务成功 / 失败经验
- 召回当前用户历史成功 / 失败案例
- 在正文评审节点注入质量提示与失败提示
- 在图片策略路由、图片需求分析、图片生成链路中参与决策
- 前端可通过调试面板查看记忆摘要与快照
